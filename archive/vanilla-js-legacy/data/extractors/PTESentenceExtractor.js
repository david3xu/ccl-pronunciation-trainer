...
export class PTESentenceExtractor {
    ...
    static extractTags(sentence) {
        ...
        if (lowerSentence.includes('?'))
            tags.push('question');
        if (lowerSentence.match(/\b(however|therefore|thus|consequently|moreover)\b/))
            tags.push('complex');
        if (lowerSentence.match(/\b(first|second|third|finally)\b/))
            tags.push('sequential');
        if (lowerSentence.match(/\b(important|essential|crucial|vital|significant)\b/))
            tags.push('emphasis');
        return tags;
    }
    /**
...
//# sourceMappingURL=PTESentenceExtractor.js.map