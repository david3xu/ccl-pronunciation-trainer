...
/**
 * Topic Terms Pipeline
 * Extracts interview topic content from topic-terms-clean.md for pronunciation training
 * Processes: data/source/topic-terms-clean.md → data/processed/topic-terms-dataset.json
 */
...
// Paths
const SOURCE_FILE = path.join(__dirname, '../data/source/topic-terms-clean.md');
const OUTPUT_DIR = path.join(__dirname, '../data/processed');
const OUTPUT_FILE = path.join(OUTPUT_DIR, 'topic-terms-dataset.json');
const REPORT_DIR = path.join(__dirname, '../data/reports');
const REPORT_FILE = path.join(REPORT_DIR, 'topic-terms-processing.txt');
...
/**
 * Extract topic content from markdown
 * Organizes by sections and preserves document structure
 */
function extractTopicContent(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!trimmed || trimmed.startsWith('# Topic Terms')) {
      continue;
    }
...
function inferDifficulty(sentence) {
  ...
  if (wordCount > 20 || length > 120 ||
    sentence.includes('PydanticAI') ||
    sentence.includes('multi-agent') ||
    sentence.includes('graph database') ||
    sentence.includes('relationship mapping') ||
    sentence.includes('Supervised Fine-Tuning') ||
    sentence.includes('Direct Preference Optimization') ||
    sentence.includes('computational biology')) {
    return 'hard';
  }

  // Short, simple sentences
  if (wordCount < 8 && length < 50) {
    return 'easy';
  }
...
function main() {
  ...
  try {
    ...
    const { sections, allSentences } = extractTopicContent(content);
    ...
    const output = {
      metadata: {
        ...
        source: 'topic-terms-pipeline',
        version: '1.0',
        description: 'Interview topic practice content for pronunciation training'
      },
      sections: sections,
...
module.exports = { extractTopicContent, inferDifficulty };
