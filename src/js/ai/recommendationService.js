 ...
 */
export async function generateRecommendations(userProgress) {
 ...
 */
function buildRecommendationPrompt(userProgress) {
    const { completedItems, totalItems, accuracy, weakAreas, recentActivity } = userProgress;
    const weakAreasText = weakAreas.length > 0
        ? weakAreas.map(area => `- "${area.word}" (${area.difficulty}): ${area.correctAttempts}/${area.attempts} correct (${Math.round(area.correctAttempts / area.attempts * 100)}%)`).join('\n')
        : 'No weak areas identified yet.';
    const recentActivityText = recentActivity.length > 0
        ? recentActivity.map(activity => `- ${activity.practiceMode}: ${activity.itemsCompleted} items (${activity.date})`).join('\n')
        : 'No recent activity.';
...
function parseAIResponse(aiResponse) {
    try {
        ...
        return recommendations.filter(rec => rec.type && rec.priority && rec.category && rec.reason).slice(0, 5); // Max 5 recommendations
    }
    catch (error) {
 ...
 */
export async function getItemFeedback(word, userAttempts, userCorrect) {
    const genAI = getGeminiClient();
    if (!genAI) {
