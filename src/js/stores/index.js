 ...
 */
export const useAppStore = create()(subscribeWithSelector(devtools(persist((set, get) => ({
    ...
    audio: {
        ...
        setAutoPlay: (autoPlayEnabled) => set((state) => ({ audio: { ...state.audio, autoPlayEnabled } })),
    ...
    tts: {
        ...
        startSpeaking: (word, phonetic, mode = 'word') => {
            ...
            if (window.analyticsService) {
                window.analyticsService.trackTTSUsed({
                    word,
        ...
        setVoice: (voice) => {
            // Track voice change
            if (window.analyticsService) {
                window.analyticsService.track('tts_voice_changed', { voice });
            }
    ...
    settings: {
        ...
        updateSetting: (key, value) => {
            ...
            if (window.analyticsService) {
                window.analyticsService.trackSettingChanged(key, value);
            }
    ...
    vocabulary: {
        ...
        setCurrentItem: (item) => {
            ...
            if (item && window.analyticsService) {
                const word = item.word || item.sentence || item.question || 'unknown';
                const difficulty = item.difficulty || item.metadata?.difficulty || 'normal';
                const category = item.category || item.metadata?.category || get().vocabulary.mode;
                window.analyticsService.trackWordPractice(word, {
                    difficulty,
                    category,
        ...
        filterByDifficulty: (difficulty) => {
            ...
            const filtered = currentDataset.filter((item) => {
                ...
                if ('metadata' in item && item.metadata?.difficulty)
                    return item.metadata.difficulty === difficulty;
                return false;
            });
    ...
    progress: {
        ...
        endSession: () => {
            ...
            if (window.analyticsService && itemsCompleted > 0) {
                window.analyticsService.trackPracticeSessionCompleted({
    ...
    onRehydrateStorage: () => (state) => {
        if (state?.progress?.completedItems) {
            state.progress.completedItems = new Set(state.progress.completedItems);
        }
    },
}), {
    name: 'PTE App Store',
