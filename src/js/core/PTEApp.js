...
export class PTEVocabularyTrainer {
    ...
    async initializeSettingsModule() {
        ...
        if (typeof window.SettingsModule !== 'undefined') {
            try {
                window.settingsModule = new window.SettingsModule(window.appConfig, window.eventBus, window.storage);
    ...
    validateModule(moduleName, moduleInstance, options = {}) {
        const { requiredProperties = [], critical = false, customCheck = null, customCheckMessage = 'Custom validation failed' } = options;
        // Check module exists
        if (!moduleInstance) {
    ...
    async initializeDatasetManager() {
        ...
        if (window.DatasetManager) {
            try {
                ...
                const config = window.appConfig || new window.AppConfig();
    ...
    initializeDataSchema() {
        ...
        if (!window.dataSchema) {
            ...
            try {
                ...
                window.dataSchema = new window.DataSchema(window.appConfig);
...
//# sourceMappingURL=PTEApp.js.map